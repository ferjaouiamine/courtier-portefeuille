// Applique le schéma de base. L'import initial doit être activé explicitement.
...
async function main() {
  ...
  try {
    ...
    if (process.env.IMPORT_DONNEES === 'true' && fs.existsSync(cheminImport)) {
      await client.query(
    ...
    } else if (process.env.IMPORT_DONNEES === 'true') {
      console.log('Aucun fichier db/import_donnees.sql trouvé — import ignoré.');
    } else {
      console.log('Import des données ignoré (utilisez IMPORT_DONNEES=true pour un import initial).');
    }
  } finally {
    await client.end();
  }
